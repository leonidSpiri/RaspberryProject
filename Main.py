            ...
            try:
                ...
                if humidity is not None and temperature is not None:
                    ...
                    with conn.cursor() as cursor:
                        ...
                        conn.close()
                db.child("home").child("rooms").child(roomStr).child("status").set(
                    str(date) + " humidity = " + str(humidity) + " temp = " + str(temperature))
            except (requests.ConnectionError, requests.Timeout) as exception:
                print("No internet connection.")
        time.sleep(5 * 60)
...
oneFanLight = threading.Thread(target=FanLightRelayWork,
                               args=(GuestBathRelayFanPin, GuestBathRelayLightPin, GuestBathKey, GuestBathStr))
